...
<main class="wrap">
  ...
  <div class="config-card">
    ...
    <svg id="config-svg" viewBox="0 0 800 170" class="config-svg" role="img" aria-label="Jupiter and its four moons" data-t0="{t0}" data-t1="{t1}"></svg>
    <input type="range" id="config-slider" min="0" max="100" value="0" step="1" aria-label="Time">
  </div>
  <p class="hint">Jupiter's equator horizontal, east to the left as in binoculars. Drag through the month, or tap any
  line in the list below to jump there. Moons behind the planet fade. Positions ±0.1 Jupiter radii — for the
  picture; the times come from JPL.</p>
...
(function () {{
  ...
  var NAMES = ['Io', 'Europa', 'Ganymede', 'Callisto'], COL = ['#f59e0b', '#60a5fa', '#a78bfa', '#34d399'], RAD_RJ = [0.0255, 0.0218, 0.0368, 0.0337];
  var half = 30, W = 800, Hh = 170, cx = W / 2, cy = Hh / 2, timer = null;
  function el(n, a) {{ var e = document.createElementNS(NS, n); for (var k in a) e.setAttribute(k, a[k]); return e; }}
  function draw() {{
    var ms = T0 + (+sl.value) * STEP, pos = galilean(ms), s = (W / 2 - 20) / half;
    ...
    function moon(p, i, dim) {{
      ...
      var t = el('text', {{ x: cx + p.x * s, y: cy - p.y * s - r - 4, 'text-anchor': 'middle', class: 'cfg-lbl' }}); t.textContent = NAMES[i][0]; g.appendChild(t);
      svg.appendChild(g);
    }}
    pos.forEach(function (p, i) {{ if (!p.front) moon(p, i, Math.abs(p.x) < 1 && Math.abs(p.y) < 1); }});
    svg.appendChild(el('ellipse', {{ cx: cx, cy: cy, rx: s, ry: s * 0.935, class: 'cfg-jup' }}));
  ...
  zoomBtn.addEventListener('click', function () {{ half = half === 30 ? 8 : 30; zoomBtn.textContent = half === 30 ? 'Zoom in' : 'Zoom out'; draw(); }});
